...
module.exports = (function () {
   ...
   function deleteBlob(regionContext, container, Blob, callback) {
      console.log('regionContext.storageAccount-'+regionContext.storageAccount)
      console.log('regionContext.storageAccessKey-'+regionContext.storageAccessKey)
      ...
      blobSvc.deleteBlob(container, Blob, function (error, response) {
         ...
         if (error) {
            callback(new AzureError('err delete Blob-' + error));
            return;
         }
         callback(null, true);
         return;
   ...
   var that = {
      ...
      deleteNode: function (settings, callback) {
         ...
         azurePostPutRequest('GET', settingsGetVm, function (err, resVm) {
            ...
            console.log('vmNic-' + vmNic)
            console.log('vmOsDisc-' + vmOsDisc)
            ...
            azurePostPutRequest('DELETE', settingsDelVm, function (err, resVm) {
               ...
               if (err) {
                  ...
                  azureRetryRequest('DELETE', settingsdelNetwork, 20, interval, function (err, resVm) {
                     ...
                     deleteBlob(settings.regionContext, 'vhds', settings.node.id+'.vhd', function (err, res) {
